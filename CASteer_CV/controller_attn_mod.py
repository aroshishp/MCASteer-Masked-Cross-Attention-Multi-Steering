...
# ── NEW: attention-weight capture (insert after imports, before VectorControl) ──

class CapturingAttnProcessor:
    """
    Wraps an existing attention processor for ONE forward call.
    Captures the softmax attention weights into `cache_target[key]`
    as a CPU tensor of shape [batch, heads, S, L], then restores the original processor.
    """
    def __init__(self, original_processor, cache_target: dict, key):
        self._orig  = original_processor
        self._cache = cache_target
        self._key   = key

    def __call__(self, attn, hidden_states,
                 encoder_hidden_states=None, attention_mask=None, **kwargs):

        # ── manual cross-attention (mirrors AttnProcessor logic) ──────────
        query = attn.to_q(hidden_states)
        kv    = encoder_hidden_states if encoder_hidden_states is not None else hidden_states
        key_proj   = attn.to_k(kv)
        value_proj = attn.to_v(kv)
        ...
        attn_weights = attn.get_attention_scores(query, key_proj, attention_mask)
        # attn_weights: [B*H, S, L]

        # store reshaped to [B, H, S, L]
        B_H, S, L = attn_weights.shape
        H = attn.heads
        B = B_H // H
        ...
        hidden_states = torch.bmm(attn_weights, value_proj)
    ...
    def forward(self, vector, place_in_unet: str):
        ...
        if self.steer:
            ...
            if place_in_unet in ['up', 'mid'] or (place_in_unet == 'down' and not self.steer_only_up): 
                ...
                if num_steer not in self.steering_vectors:
                    return vector
    ...
    def block_forward(self, place_in_unet):
        ...
        def forward(
            ...
            if self.attn2 is not None:
                ...
                # ── capture attn weights if controller requests it ────────
                layer_idx = len(controller.step_store[place_in_unet])
                cache_key = (place_in_unet, layer_idx)
                attn_cache = getattr(controller, 'attn_weight_cache', None)

                if attn_cache is not None:
                    orig_proc = self.attn2.processor
                    self.attn2.processor = CapturingAttnProcessor(
                        orig_proc, attn_cache, cache_key
                    )
                ...
                if attn_cache is not None:
                    self.attn2.processor = orig_proc   # restore immediately
